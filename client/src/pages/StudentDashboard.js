...
export default function StudentDashboard() {
  ...
  const fetchResults = async () => {
    ...
    try {
      ...
      if (data.results.length > 0 && !selectedResult) setSelectedResult(data.results[0]);
    } catch (err) {
      console.error(err);
  ...
  const getGradeColor = (grade) => {
    if (['A'].includes(grade)) return 'badge-green';
    if (['B', 'C'].includes(grade)) return 'badge-gold';
    return 'badge-red';
  };
  ...
  return (
    <div className="student-portal">
      ...
      {/* Tabs */}
      <div className="result-tabs" style={{ marginBottom: 20 }}>
        <button
        ...
        <button
          className={`result-tab ${activeTab === 'assignments' ? 'active' : ''}`}
      ...
      {activeTab === 'assignments' && (
        loading ? (
          ...
          <div className="empty-state card">
            ...
            <p>Your teacher hasn't posted any assignments for your class. Check back soon!</p>
          </div>
        ) : (
            ...
            {assignments.map((a, idx) => (
              <div key={a._id} style={{
                ...
                <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{
                    width: 42, height: 42, borderRadius: 10, flexShrink: 0, marginTop: 2,
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 16px', fontSize: '0.78rem', color: '#6b7280', marginBottom: a.description ? 8 : 0 }}>
                      ...
                      {a.dueDate && (
                        <span style={{ color: '#c9a84c', fontWeight: 600 }}>
                          <i className="fa-solid fa-calendar-days" style={{ marginRight: 4 }}></i>Due: {a.dueDate}
                        </span>
                      )}
                    ...
                    {a.description && (
                      <p style={{ fontSize: '0.85rem', color: '#374151', margin: 0, lineHeight: 1.6, whiteSpace: 'pre-line' }}>
                        {a.description}
                      </p>
                    )}
                  </div>
        ...
        <>
            ...
            <div className="result-card card" ref={printRef}>
                ...
                <div className="result-school-info">
                  ...
                  <div className="result-type-badge">
                    {(() => {
                      ...
                      if (cn.startsWith('Grade'))   return 'PRIMARY TERMINAL REPORT';
              ...
              <div className="result-meta-grid">
                ...
                <div className="meta-row">
                  ...
                  <div className="meta-field"><span>Student Average</span><strong>{selectedResult.studentAverage || (selectedResult.percentage ? selectedResult.percentage + '%' : '—')}</strong></div>
                  <div className="meta-field"><span>Overall Result</span><strong className={selectedResult.overallResult === 'PASS' ? 'text-green' : 'text-red'}>{selectedResult.overallResult || 'PASS'}</strong></div>
                </div>
              </div>
                ...
                <>
                        ...
                        {(selectedResult.subjects || []).map((s, i) => {
                          ...
                          return (
                            <tr key={i}>
                              ...
                              <td style={{ textAlign: 'center' }}><strong>{s.grade || '—'}</strong></td>
                              <td>{s.remark || '—'}</td>
                  ...
                  {selectedResult.obtainedMarks > 0 && (
                    <div className="total-score-row">
                      <strong>Total Score: {selectedResult.totalScore || selectedResult.obtainedMarks}</strong>
                    </div>
                  )}
                  ...
                  <div className="cognitive-keys">
                    <strong>Cognitive Keys</strong><br/>
          ...
          {selectedResult && (
            <div className="print-bar">
              <button className="btn btn-primary" onClick={handlePrint}><i className="fa-solid fa-print"></i> Print Report Card</button>
            </div>
          )}
        </>
        )}
